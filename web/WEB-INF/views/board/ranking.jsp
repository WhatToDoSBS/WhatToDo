...
<div class="ratingContainer">
    ...
    <div>
        <table>
            ...
            <c:forEach items="${requestScope.ratingList}" var="item">
                <tr>
                    <td>
                            <%--eq는 문자열이나 숫자가 같다는 의미--%>
                        <c:choose>
                            <c:when test="${item.gameRank eq '1'}"><i class="fas fa-crown gold"></i></c:when>
                            <c:when test="${item.gameRank eq '2'}"><i class="fas fa-crown sliver"></i></c:when>
                            <c:when test="${item.gameRank eq '3'}"><i class="fas fa-crown bronze"></i></c:when>
                        </c:choose>
                        <c:out value="${item.gameRank}"/></td>
                    <td><c:out value="${item.gameNm}"/></td>
                    <td><c:out value="${item.gameRating}"/></td>
                </tr>
            </c:forEach>
        </table>
    </div>
